...
iris = pd.read_csv('iris.data', header=None, sep=',')  # 读取文件
iris1 = iris.iloc[0:150, 0:4]  # 获取前150行，前4列
iris2 = np.mat(iris1)  # 转换为矩阵

X1 = iris2[0:50, 0:4]  # 取前50个数据，即setosa类
X2 = iris2[50:100, 0:4]  # 取50~100个数据，即versicolor类
X3 = iris2[100:150, 0:4]  # 取后面50个数据，即virginica类
...
def GA(d):
    ...
    population = np.zeros((n, 4))  # 初始化种群，初始种群数量为50，特征全部为0
    for i in range(n):  # 定义种群的个体数为 n
        a = np.zeros(4 - d)  # 生成未被选择的特征
...
def crossover(population):
    ...
    for i in range(10):
        ...
        one_zero = []
        zero_one = []
        for j in range(4):  # 遍历每一个特征，iris一共四个特征
